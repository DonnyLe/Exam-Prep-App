...
export default function SelectForm({ user_id }: { user_id: string }) {
  const onSubmit = async (data: ExamFormData) => {
    console.log("entered 2");
    await addExam(data, user_id);
  };
  ...
  const [supabase, setSupabase] = useState<SupabaseClient>(createClient());

  const [new_subject, setNewSubject] = useState("");

  const getSubjects = async () => {
    const subjects = await fetchSubjects(supabase, user_id);
  ...
  let addSubject = async () => {
    if (new_subject) {
      let subject_id = await supabase
        .from("subjects")
        .insert([{ subject_name: new_subject, user_id: user_id }])
        .select("id");
      getSubjects();
    }
  };
